                                                ...
                                                int strct_id)
{
    ...
    if (!from_restart)
    {
        ...
        if (modified_box)
        {
            pout << "CustomIBHydrodynamicForceEvaluator::registerStructure: integration box modified from\n"
    ...
    else
    {
        ...
        {
            TBOX_ERROR("CustomIBHydrodynamicForceEvaluator::registerStructure(). Restart database corresponding to "
...
void
CustomIBHydrodynamicForceEvaluator::updateStructureDomain(const IBTK::Vector3d& box_vel_new,
                                                    double dt,
                                                    Pointer<PatchHierarchy<NDIM> > /*patch_hierarchy*/,
                                                    int strct_id)

{
    ...
    CustomIBHydrodynamicForceEvaluator::IBHydrodynamicForceObject& fobj = d_hydro_objs[strct_id];
...
void
CustomIBHydrodynamicForceEvaluator::setTorqueOrigin(const IBTK::Vector3d& X0, int strct_id)
{
...
void
CustomIBHydrodynamicForceEvaluator::preprocessIntegrateData(double /*current_time*/, double /*new_time*/)
{
    pout << "WARNING:: CustomIBHydrodynamicForceEvaluator::preprocessIntegrateData() not implemented.\n";

    return;
} // preprocessIntegrateData

const CustomIBHydrodynamicForceEvaluator::IBHydrodynamicForceObject&
CustomIBHydrodynamicForceEvaluator::getHydrodynamicForceObject(int strct_id)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(d_hydro_objs.find(strct_id) != d_hydro_objs.end());
#endif

    const CustomIBHydrodynamicForceEvaluator::IBHydrodynamicForceObject& force_obj = d_hydro_objs[strct_id];
    return force_obj;

} // getHydrodynamicForceObject

void
CustomIBHydrodynamicForceEvaluator::computeLaggedMomentumIntegral(
    int u_old_idx,
    Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
...
void
CustomIBHydrodynamicForceEvaluator::updateStructureMomentum(const IBTK::Vector3d& P_strct_new,
...

{
    ...
    CustomIBHydrodynamicForceEvaluator::IBHydrodynamicForceObject& force_obj = d_hydro_objs[strct_id];
...
void
CustomIBHydrodynamicForceEvaluator::computeHydrodynamicForce(int u_idx,
                                                       int p_idx,
                                                       int /*f_idx*/,
                                                       Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
                                                       double dt,
...
void
CustomIBHydrodynamicForceEvaluator::postprocessIntegrateData(double /*current_time*/, double new_time)
{
    for (auto& hydro_obj : d_hydro_objs)
    {
...
void
CustomIBHydrodynamicForceEvaluator::putToDatabase(SAMRAI::tbox::Pointer<SAMRAI::tbox::Database> db)
{
    for (const auto& hydro_obj : d_hydro_objs)
    {
        ...
        // Store separated force components for restart
...
void
IBAMR::CustomIBHydrodynamicForceEvaluator::registerStructurePlotData(Pointer<VisItDataWriter<NDIM> > visit_data_writer,
                                                               Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
                                                               int strct_id)
...
void
IBAMR::CustomIBHydrodynamicForceEvaluator::updateStructurePlotData(Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
                                                             int strct_id)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(d_hydro_objs.find(strct_id) != d_hydro_objs.end());
...
void
CustomIBHydrodynamicForceEvaluator::resetFaceAreaWeight(Pointer<PatchHierarchy<NDIM> > patch_hierarchy)
{
    const int coarsest_ln = 0;
    const int finest_ln = patch_hierarchy->getFinestLevelNumber();
...
void
CustomIBHydrodynamicForceEvaluator::resetFaceVolWeight(Pointer<PatchHierarchy<NDIM> > patch_hierarchy)
{
    const int coarsest_ln = 0;
    const int finest_ln = patch_hierarchy->getFinestLevelNumber();
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
...
void
CustomIBHydrodynamicForceEvaluator::fillPatchData(const int u_src_idx,
                                            const int p_src_idx,
                                            Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
                                            const std::vector<RobinBcCoefStrategy<NDIM>*>& u_src_bc_coef,
                                            RobinBcCoefStrategy<NDIM>* p_src_bc_coef,
...
void
CustomIBHydrodynamicForceEvaluator::getPhysicalCoordinateFromSideIndex(IBTK::Vector3d& side_coord,
